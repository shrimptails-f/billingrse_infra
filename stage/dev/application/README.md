...
- front bucket の `force_destroy` は module 変数で切り替え可能とし、`dev` 環境では `true` を渡す
...
- 環境ごとに front bucket の削除方針を切り替えられるようにしつつ、`dev application` の destroy 時はオブジェクトが残っていても stack を破棄できるようにするため
- 開発環境のフロント配信物は再デプロイ前提であり、bucket 内オブジェクトを destroy 時に保持する運用価値が低いため
- VS Code task や運用手順に追加の bucket empty 処理を持ち込まず、Terraform の責務だけで削除を完結させるため
...
この root stack は `outputs.tf` を持たず、remote state 経由で他 stack に値を公開しない設計とする。

- `stage/dev/application` の root outputs は定義しない
- ALB DNS 名、ECS cluster 名、RDS endpoint などの値は module 内では生成されるが、root stack からは再公開しない
- 後続の運用や参照が必要な値は、必要になった時点で root outputs へ明示的に昇格させる
